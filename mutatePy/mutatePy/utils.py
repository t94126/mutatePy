...
def write_to_file(ast_node, filename, mutant_count, indent_number = 2):
    dir = ".//mutatedCode//"+str(mutant_count)
    if not os.path.exists(dir):
        os.makedirs(dir)
    code = astor.to_source(ast_node, indent_with=' ' * indent_number)
    f = open(dir+"//"+filename+".py", 'w')
    f.write(code)
    f.close()
