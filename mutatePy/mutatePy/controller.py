...
class MutationController(views.ViewNotifier):
    ...
    def __init__(self, target_loader, views, mutant_generator,
    ...
    def run(self):
        self.notify_initialize(self.target_loader.names)
        try:
            timer = utils.Timer()
    ...
    def run_mutation_process(self):
        try:
            total_duration = 0;
            self.notify_start()
            for target_file_name, to_mutate in self.target_loader.load():
                self.mutate_module(target_file_name, to_mutate, total_duration)
        except KeyboardInterrupt:
            pass

    # @utils.TimeRegister
    def mutate_module(self, target_file_name, to_mutate, total_duration):
        target_ast = self.create_target_ast(target_file_name)
        filename = target_file_name
        file_suffix = 0
        utils.write_to_file(target_ast, filename, file_suffix)
        file_suffix = file_suffix+1
        for mutations, mutant_ast in self.mutant_generator.mutate(target_ast, to_mutate):
            mutation_number = self.score.all_mutants
            self.notify_mutation(mutation_number, mutations, target_file_name, mutant_ast)
            utils.write_to_file(mutant_ast, filename, file_suffix)
            file_suffix = file_suffix + 1
